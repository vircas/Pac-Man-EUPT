...
class KeyboardAgent(object):
    ...
    def getAction(self, state):
        from graphics.graphicsUtils import keys_waiting
        from graphics.graphicsUtils import keys_pressed
...
class IntelligentAgent(object):
    """
        An agent controlled by a search algorithm.
    """
    def __init__(self, index=0):
        self.index = index
...
class GhostAgent(Agent):
    ...
    def getAction(self, state):
        legal = Actions.getPossibleActions(state.getGhostState(self.index).configuration, state.data.layout.walls)
